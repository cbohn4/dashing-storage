...
def main():
    ...
    for line in stdoutdata.split("\n"):
        ...
        if split_line[5] == MOUNT:
            dash = dashing.DashingImport('viz.unl.edu', port=3030,auth_token = auth_key)            
    ...
    time.sleep(1)
    dash.SendEvent('AnvilTile', {'current_mem': str(round(int(rawData["mem_count"])/(1024.0**2),2))})
    time.sleep(1)
    dash.SendEvent('AnvilTile', {'current_vol': str(round(int(rawData["volume_gb"])/1024.0,2))})
    time.sleep(1)
